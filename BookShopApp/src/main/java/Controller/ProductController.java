...
public class ProductController {
	public List<ProductClass> findallProducts() throws ClassNotFoundException, SQLException{
		...
		while(resultset.next()) {
			System.out.println(resultset.getInt("id")+"\t\t"+resultset.getInt("age")+"\t\t"+resultset.getFloat("salary")+"\t\t"+resultset.getString("gender"));
			
		}
		return productList;
	}
	public int save() throws ClassNotFoundException, SQLException {
		String query="insert into dummy1(id,,age,salary,gender)values(?,?,?,?)";
		int rows_inserted=0;
		   try {
